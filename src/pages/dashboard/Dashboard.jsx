        ...
        <div className="dash-foot">
          <div className="dash-foot-user">
            ...
            <div className="dash-foot-user-info">
              ...
              <div className="dash-foot-role">Administrator</div>
            </div>
                    ...
                    <table className="dash-patient-table">
                      ...
                      <tbody>
                        {filteredPatients.slice(0,6).map(p => (
                          <tr key={p.id}>
                            <td><div className="dash-patient-name"><span>{String(p.full_name || 'P').charAt(0).toUpperCase()}</span>{p.full_name}</div></td>
                            <td>{p.age || '—'}</td>
                            ...
                            <td><button className="dash-more"><Icon name="more" size={17}/></button></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            <div className="dash-panel">
                ...
                <table className="dash-full-table">
                  ...
                  <tbody>
                    {filteredPatients.map(p => {
                      ...
                      return (
                        <tr key={p.id}>
                          <td onClick={() => setSelectedOrdersPatient(isSelected ? '' : p.full_name)} className={isSelected ? 'selected-patient' : ''}>{p.full_name}</td>
                          <td>{p.age}</td>
                          <td><span className={`dash-status ${p.status === 'review' ? 'review' : 'stable'}`}>{p.status === 'review' ? 'In Review' : 'Stable'}</span></td>
              ...
              {selectedOrdersPatient && (
                <div className="dash-orders">
                  <div className="dash-panel-head">
                    <div className="dash-panel-title">Doctor's Orders — {selectedOrdersPatient}</div>
                    <button className="btn btn-ghost" onClick={() => setSelectedOrdersPatient('')}>Clear</button>
                  </div>
                  {(() => {
                    const orders = prescriptions.filter(rx => rx.patient_name === selectedOrdersPatient && rx.status === 'active')
                    if (!orders.length) return <div className="dash-empty">No active doctor's orders.</div>
                    return <ul className="dash-legend">{orders.map(rx => <li key={rx.id}><span className="dash-legend-name"><span className="dash-legend-dot" style={{background:'var(--teal)'}}/>{rx.drug_name} — {rx.dosage}</span><span className="dash-legend-val">{rx.frequency || '—'}</span></li>)}</ul>
                  })()}
                </div>
              )}
            </div>
