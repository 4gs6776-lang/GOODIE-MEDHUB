...
export default function Reception(){
  ...
  const searchMatches = queueSearch
    ? inQueue
        .filter(patient => {
          ...
          const phoneNumber = String(patient.phone || '').toLowerCase()
  ...
  function selectPatient(patient){
    setSearchTerm(patient.patient_id || patient.full_name || '')
    ...
    <>
      <div className="dash-panel" style={{ marginBottom: 16 }}>
        ...
        >
          <span>
            {queueSearch
              ? `${visibleQueue.length} matching patient(s)`
              : `${inQueue.length} patient(s) currently in the queue`}
          </span>
      ...
      {loading ? (
        ...
        <div className="dash-row dash-row-2b">
          ...
          {QUEUE_STAGES.map(stage => {
            ...
            const stagePatients = visibleQueue.filter(
              p => p.queue_status === stage.key
            )
