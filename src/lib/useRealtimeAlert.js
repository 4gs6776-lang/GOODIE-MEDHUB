...
//
// `event` (optional, default 'INSERT') also accepts 'UPDATE' — used by
// the Doctor Workbench to catch lab results being COMPLETED (an update
// issued by the laboratory, not a new row). Requires replica identity
// full + publication membership (migrations 006/008) for UPDATE events
// to carry the full row.
//
...
export function useRealtimeAlert(tableName, hospitalId, onInsert, { event = 'INSERT' } = {}) {
  ...
  useEffect(() => {
    ...
    const channelName = `realtime-${tableName}-${event}-${hospitalId}-${Math.random().toString(36).slice(2)}`
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          const row = event === 'UPDATE' ? payload?.new : (payload?.new || undefined)
          if (row && typeof onInsertRef.current === 'function') {
